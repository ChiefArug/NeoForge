...
class TimeSpeedCommand {
    ...
    private static int query(CommandSourceStack source) {
        ...
        if (speed < 0) {
            source.sendSuccess(() -> Component.translatableWithFallback("commands.neoforge.timespeed.query.default", "Time in %s flows normally (20 minutes per day).", levelName(source)), true);
        } else {
            source.sendSuccess(() -> Component.translatableWithFallback("commands.neoforge.timespeed.query", "Time in %s flows at a rate of %sx (%s minutes per day).", levelName(source), speed, minutes(speed)), true);
        }
    ...
    private static int setSpeed(CommandSourceStack source, float speed) {
        ...
        if (!rule.get() && speed > 0) {
            rule.set(true, null);
            source.sendSuccess(() -> Component.translatableWithFallback("commands.gamerule.set", GameRules.RULE_DAYLIGHT.getId(), rule.toString()), true);
        ...
        source.sendSuccess(() -> Component.translatableWithFallback("commands.neoforge.timespeed.set", "Set flow of time in %s to %sx (%s minutes per day).", levelName(source), speed, minutes(speed)), true);
        return Command.SINGLE_SUCCESS;
    }
    ...
    private static int setDefault(CommandSourceStack source) {
        ...
        source.sendSuccess(() -> Component.translatableWithFallback("commands.neoforge.timespeed.set.default", "Set flow of time in %s to default (20 minutes per day).", levelName(source)), true);
        return Command.SINGLE_SUCCESS;
    }
}
